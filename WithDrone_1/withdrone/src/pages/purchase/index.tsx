...
export default function Purchase() {
  ...
  const handlePurchaseClick = () => {
    //order post 보내고 확정페이지로
    if (count === 1) {
      //장바구니
      makeCartOrder(orderInfo).then((fetchedData) => {
      ...
      makeOneOrder(orderInfo, productId).then((fetchedData) => {
        console.log(fetchedData);
        navigate(PATH.PURCHASE_CONFIRM, { state: fetchedData });
      });
    }
  ...
  return (
    <S.Container>
      ...
      <S.PayButton onClick={handlePurchaseClick} disabled={!isInputFilled()}>
        결제하기
      </S.PayButton>
    </S.Container>
  );
