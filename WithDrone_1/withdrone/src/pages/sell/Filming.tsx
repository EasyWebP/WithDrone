...
export default function Filming() {
  ...
  useEffect(() => {
    fetchProductList("촬영용", undefined).then(
      (fetchedData) => {
  ...
  useEffect(() => {
    fetchProductList("촬영용", likeState, priceState, undefined, {page:activePage-1,size:PAGENUM}).then(
      (fetchedData) => {
  ...
  const handlePageChange = (pageNumber: number) => {
    ...
    fetchProductList("촬영용", likeState, priceState, undefined, {page:pageNumber-1,size:PAGENUM}).then(
      (fetchedData) => {
        setDroneLists(fetchedData.content);
      }
    );
