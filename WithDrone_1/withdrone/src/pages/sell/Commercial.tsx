...
export default function Commercial() {
  ...
  useEffect(() => {
    fetchProductList("상업용", undefined).then(
      (fetchedData) => {
  ...
  useEffect(() => {
    fetchProductList("상업용", likeState, priceState, undefined, {page:activePage-1,size:PAGENUM}).then(
      (fetchedData) => {
  ...
  const handlePageChange = (pageNumber: number) => {
    ...
    fetchProductList("상업용", likeState, priceState, undefined, {page:pageNumber-1,size:PAGENUM}).then(
      (fetchedData) => {
        setDroneLists(fetchedData.content);
      }
    );
