...
export default function Child() {
  ...
  useEffect(() => {
    fetchProductList("어린이용", undefined, undefined, "RENTAL").then(
      (fetchedData) => {
  ...
  useEffect(() => {
    fetchProductList("어린이용", likeState, priceState, "RENTAL", {page:activePage-1,size:PAGENUM}).then(
      (fetchedData) => {
  ...
  const handlePageChange = (pageNumber: number) => {
    ...
    fetchProductList("어린이용", likeState, priceState, "RENTAL", {page:pageNumber-1,size:PAGENUM}).then(
      (fetchedData) => {
        setDroneLists(fetchedData.content);
      }
    );
